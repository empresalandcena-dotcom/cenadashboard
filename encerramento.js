...
window.renderEncerramentoCharts = function renderEncerramentoCharts(C) {
  ...
  mkChart('ch8', {
    ...
    options: {
      ...
      plugins: {
        legend: { display: false },
        valueLabelPlugin: { enabled: true, color: C.text, fontSize: 12, datasetIndexes: [0, 1, 2], rotate: -90, rotateYOffset: 38, formatter: (value) => fmtCurrencyCompact(value) },
      },
      scales: {
...
function encRenderCascataStatus(C) {
  ...
  mkChart('ch13', {
    ...
    options: {
      ...
      plugins: {
        legend: { display: false },
        valueLabelPlugin: { enabled: true, color: C.text, fontSize: 12, datasetIndexes: [0, 1, 2], rotate: -90, rotateYOffset: 38, totalInside: true, totalInsideOffset: 4, formatter: (value) => fmtCurrencyCompact(value) },
      },
      scales: {
...
function encRenderTable(rows) {
  ...
  const filtered = columnFiltered.filter((row) => {
    ...
    return [row.nota, row.municipio, row.responsavel, row.pep].join(' ').toLowerCase().includes(query);
  });
